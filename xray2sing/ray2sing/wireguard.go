...
func convertRange(s string) warpobf.Range {
	r := warpobf.Range{}
	r.UnmarshalJSON([]byte(strconv.Quote(s)))
	return r
}
func getWireGuardNoise(d map[string]string, addDefault bool) warpobf.NoiseOptions {
	...
	if fake_packet_count.To == 0 && fake_packet_delay.To == 0 && fake_packet_size.To == 0 && fake_packet_mode == "" {
		...
		return warpobf.NoiseOptions{}
	}
	return warpobf.NoiseOptions{
		FakePacket: warpobf.FakePacketOptions{
			Enabled: true,
...
func defaultWireguardNoiseOptions() warpobf.NoiseOptions {
	return warpobf.NoiseOptions{
		FakePacket: warpobf.FakePacketOptions{
			Enabled: true,
